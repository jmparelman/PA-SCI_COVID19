...
d3.csv("https://raw.githubusercontent.com/jmparelman/PA-SCI_COVID19/main/data/latest_data/PA_DOC_Daily_Counts.csv").then(function(data){


    // daily sums
    var summary_data = d3.rollups(data, v => ({
                        'ip_active_cases': d3.sum(v, d => d.incarcerated_person_active_positive) + d3.sum(v, d => d.incarcerated_person_asymptomatic_positive),
...
54								// update for d3 v6
							 data.forEach(function(d) {
									 ...
									 d.incarcerated_person_active_cases = parseFloat(0+d.incarcerated_person_active_positive) + parseFloat(0+d.incarcerated_person_asymptomatic_positive);
							 });
    ...
    function drawChart() {
        ...
        x_bar.range([25, currentWidth - 30]).padding(0.4);
